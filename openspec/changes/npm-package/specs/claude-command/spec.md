...
- **WHEN** user invokes `/tracecart source.md target.md`
...
- **WHEN** user invokes `/tracecart source.md target.md --lang en`
...
The command markdown SHALL instruct Claude Code to use the Agent tool for LLM steps (extraction and matching), passing the prompt from `tracecart extract-prompt` or `tracecart match-prompt`.

#### Scenario: Extraction subagent
- **WHEN** the recipe reaches the extract step
- **THEN** Claude Code runs `tracecart extract-prompt` to get the prompt, spawns a subagent with that prompt, and saves the subagent's response to a temp file for `tracecart extract-validate`

#### Scenario: Match subagent
- **WHEN** the recipe reaches the match step
- **THEN** Claude Code runs `tracecart match-prompt` to get the prompt, spawns a subagent, and saves the response for `tracecart match-validate`
...
- **WHEN** `tracecart remainder` reports 0 uncovered clauses after first extraction
- **THEN** pipeline proceeds to match step (no loop)

#### Scenario: Remainder shrinks
- **WHEN** `tracecart remainder` reports uncovered clauses but fewer than previous iteration
...
The command markdown SHALL instruct Claude Code to report the final result after `tracecart finalize`.
...
The command markdown SHALL include a version comment that `tracecart update` can read.

#### Scenario: Version comment present
- **WHEN** the command markdown is installed via `tracecart init`
- **THEN** the file contains a comment like `<!-- tracecart v0.1.0 -->` that identifies the template version
