...
- [ ] 1.1 Create config schema and loader: read `.tracecart/config.json`, validate mappings (name uniqueness, required fields), glob pattern parsing
...
- [ ] 2.2 Implement content-hash caching: sha256 of all mapped file contents, store in `.tracecart/cache/{mapping-name}.hash`, skip pipeline on cache hit
...
- [ ] 4.2 Implement trace-map.json file output to `.tracecart/output/{mapping-name}/trace-map.json`
...
- [ ] 6.3 Document setup in CLAUDE.md: how to configure tracecart hook for a project
